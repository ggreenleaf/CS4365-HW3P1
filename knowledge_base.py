	...
	def resolution(self):
		...
		while True:
				...
				for (c1,c2) in pairs: 
					...
					new_clauses.update(set(resolve_parents)) #update new_clauses without adding repeated clauses

				if set([x[0] for x in new_clauses]).issubset(self.get_kb_set()): #if no new clauses have been added then clause tested is not valid
					return False
