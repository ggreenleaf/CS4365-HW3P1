class KnowledgeBase:
	...
	def resolve(self,c1,c2):
		...
		# print "in resolve c1:",c1,"c2:",c2
		clauses = []
		for a1 in c1[1].split(" "):
			for a2 in c2[1].split(" "):
				if a1 == self.negate_literal(a2):
					new_clauses = list(set(self.remove(a1,c1) + self.remove(a2,c2))) #remove repated literals 
					if not new_clauses: #no clauses generated after removing
						new_clauses = ["False"]
					clauses.extend([x for x in new_clauses])
					
		# print "resolved clauses: ",clauses
		return clauses
	...
	def get_kb_set(self):
		'''returns a the set of the kb each element is (clause,parents)'''
		...
		while True:
				...
				for (c1,c2) in pairs: 
					...
					if "False" in resolvents:
						breakFlag = True #add clauses to kb before exiting loop
						
					# print new_clauses.issubset(set([x[1] for x in self.kb]))
					# print "resolved: ",resolvents
					resolved_set = [(x, "{%i %i}"%(c1[0],c2[0])) for x in resolvents]
					# print resolved_set
					new_clauses.update(resolved_set)
					# print "new_clauses:", new_clauses
					# print "kb_set: ", self.get_kb_set()
					
				# if new_clauses.issubset(self.get_kb_set()): return False
				if set([x[0] for x in new_clauses]).issubset(self.get_kb_set()): return False
